...
import { sendEmailViaInternalSender } from "./utils/emailSender";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const {
      ...
      originalResponse,
      updateRequest,
      emailBody, // For SendEmailDirectly
      to, // For SendEmailDirectly
      prompt,
      systemPrompt,
      model,
      files,
    ...
    // Handle direct email sending if action is SendEmailDirectly
    if (action === "SendEmailDirectly") {
      ...
      if (sendResult.success) {
        // Optionally log this action to the database or a different collection
    ...
    // For Draft and Update actions that involve generation/regeneration
    ...
    if (action === "Draft") {
      ...
      // Use emailTitle from the original request if available, or let it be part of updateRequest context
      ...
      async start(controller) {
        ...
        if (insertedId && collection) {
          try {
            ...
            // This part is now handled by the SendEmailDirectly action and frontend logic
            // if (action === "Send") { ... }
          } catch (dbError) {
